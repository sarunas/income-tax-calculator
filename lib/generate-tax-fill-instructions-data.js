...
export const generateTaxFillInstructionsData = (report, splitGainWithPartner = false) => {
  const currentYear = new Date().getFullYear();
  const years = uniq(concat(keys(report.incomeByYear), keys(report.gainByYear))).sort();
  const instructions = {};

  for (const year of years) {
    instructions[year] = {};
    instructions[year].heading =
      year === currentYear ? "Preliminary Tax Declaration of " : "Tax Declaration of " + year;
