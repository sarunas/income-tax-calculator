...
const { fetchExchangeRage } = require('./currency-exchange');
const fetchExchangeRageMemoized = _.memoize(fetchExchangeRage);
...
const generateReport = async (issuedShares, soldShares) => {
  ...
  for (const share of issuedSharesSortedByDate) {
    ...
    share.exchangeRate = await fetchExchangeRageMemoized(date, 'USD');
  ...
  for (const transaction of soldSharesSortedByDate) {
    ...
    transaction.exchangeRate = await fetchExchangeRageMemoized(date, 'USD');
    ...
    for (const share of shareGroups[transaction.grantNumber]) {
      if (share.balance > 0) {
        if (share.balance > sharesSold) {
          ...
          gain -= round(share.cost / share.vestedShares * sharesSold);
          sharesSold = 0;
        } else {
          sharesSold -= share.balance;
          gain -= round(share.cost / share.vestedShares * share.balance);
          share.balance = 0;
        }
      }
