...
const generateReport = async (issuedShares, soldShares, fetchExchangeRate) => {
  ...
  for (const share of issuedSharesSortedByDate) {
    ...
    incomeByYear[year].taxAmount += round(
      share.incomeAmount * getIncomeTaxRate(share.vestingDate)
    )
  ...
  for (const transaction of soldSharesSortedByDate) {
    ...
    gainByYear[year].taxAmount += round(
      transaction.gain * getGainTaxRate(transaction.orderDate)
    )
